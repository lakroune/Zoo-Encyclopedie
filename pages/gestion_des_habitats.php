	...
	<div class="relative flex h-auto min-h-screen w-full group/design-root ">
		...
		<div class="flex-1 flex flex-col">
			...
			<main class="flex-1 p-4 sm:p-6 lg:p-8 position : relative">
				<div class=" mt-[-35px] p-[15px] z-10 position: fixed  left-[260px] right-[30px] bg-background-light    flex flex-wrap justify-between items-start gap-4 mb-6">
					...
					<form action="./gestion_des_habitats.php" method="POST">
				...
				<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6">
					...
					<?php }
						...
						<div class="col-span-full">
							<div class="flex flex-col items-center justify-center p-10 bg-white dark:bg-gray-900/50 rounded-xl border border-dashed border-primary/40 dark:border-primary/50 shadow-inner">

								<div class="w-16 h-16 mb-4 flex items-center justify-center rounded-full bg-[red]/20">
									<span class="material-symbols-outlined text-[red] text-4xl">eco</span>
								</div>

								<h2 class="text-xl font-bold text-gray-900 dark:text-white mb-2">
									Aucun Habitat Enregistré
								</h2>

								<p class="text-gray-600 dark:text-gray-400 mb-6 text-center max-w-md">
									Pour associer des animaux, vous devez d'abord créer au moins un habitat.
								</p>

								<span href="ajouter-habitat.php" class="inline-flex items-center justify-center gap-2 rounded-lg h-11 px-5   font-bold hover:opacity-90 transition  ">
			...
			<section class="  flex-1 p-6 lg:p-10 <?= $hidden ?> fixed left-[260px]   inset-0 bg-background-light dark:bg-background-dark z-50 overflow-y-auto"
			...
			if ($_SERVER['REQUEST_METHOD'] === 'GET') {
				if (isset($_GET['error'])) {
					if ((int)$_GET["error"] == 2) {
						...
						echo " alert('Erreur sur le serveur (name of habitat is Duplicate)') ";
					}
				}
				if (isset($_GET['upload'])) {
					if ((int)$_GET["upload"] == 1) {
						...
						echo " alert('Sélectionnez une image pour l\'habitat') ";
					}
				}
				if (isset($_GET['success'])) {
					if ((int)$_GET["success"] == 1) {
						echo "  alert('L\'habitat a été ajouté avec succès !')";
					} else if ((int)$_GET["success"] == 2) {
						echo "  alert('L\'habitat a été modifie avec succès !')";
					} else if ((int)$_GET["success"] == 3) {
						echo "  alert('L\'habitat a été supprimé avec succès !')";
					}
				}
			}
			?>
				...
				if (confirm('Voulez-vous vraiment supprimer cet habitat ?')) {
					...
					$.ajax({
						...
						success: function(response) {
							...
							location.href = './gestion_des_habitats.php?success=3';
