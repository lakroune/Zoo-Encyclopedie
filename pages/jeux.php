...
<body class="bg-background-light dark:bg-background-dark font-display">
	<div class="relative flex h-auto min-h-screen w-full flex-col">
		<div class="flex h-full min-h-screen">
			...
			<?php if (count($array_jeux) == 3) { ?>
				<main class="w-[1000px]">
					...
					<div class="flex px-4 py-3 justify-center">
						<button onclick="readText()" class="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-full h-12 px-5 bg-primary text-text-light gap-2 text-base font-bold leading-normal tracking-[0.015em] hover:opacity-90 transition-opacity">
							<span class="material-symbols-outlined text-white/90">volume_up</span>
			...
			<?php } else { ?>
				<main class=" w-[1000px] px-4 sm:px-6 lg:px-8 py-8 md:py-12">
					<div class="mx-auto max-w-7xl text-center py-16">

						<div class="w-24 h-24 mx-auto mb-6 flex items-center justify-center rounded-full bg-yellow-100 dark:bg-yellow-900/50">
							<span class="material-symbols-outlined text-yellow-600 dark:text-yellow-400 text-5xl">warning</span>
						</div>
						...
						<p class="text-lg text-text-muted-light dark:text-text-muted-dark mb-8 max-w-lg mx-auto">
							Pour lancer le Jeu Animal, nous avons besoin d'au moins trois (3) animaux différents dans la base de données.
						</p>
